...
* **Email:** django.core.mail, подтверждение регистрации через токен
* **Кэширование:** Django cache framework (Redis или Memcached)
* **Асинхронность:** Celery + Redis (отправка email, уведомления)
...
10. **notifications** — внутренние уведомления (type, title, message, is\_read)
...
3. Отправляется email с токеном подтверждения (Celery-задача)
...
4. Психологу отправляется уведомление (email + notifications)
...
* Email — отправка через Celery-задачи (tasks.py)
...
* \[ ] Асинхронность (доп.) → Celery для email и уведомлений
